...
try {
    ...
    header('Content-Type: text/csv; charset=utf-8'); // Ujistěte se, že charset je utf-8
    header('Content-Disposition: attachment; filename="ucastnici_'. $nazev_dilny_pro_soubor .'_'.date('Y-m-d').'.csv"');

    $output = fopen('php://output', 'w');

    // Přidání UTF-8 BOM (Byte Order Mark) - toto je KLÍČOVÉ pro správné otevření UTF-8 CSV v Excelu
    fwrite($output, "\xEF\xBB\xBF"); // Nebo použijte: fprintf($output, chr(0xEF).chr(0xBB).chr(0xBF));

    // Hlavička CSV - explicitně nastavíme středník jako oddělovač
    fputcsv($output, array('Jméno rodiče', 'Jméno dítěte', 'Datum přihlášení'), ';');

    // Data - explicitně nastavíme středník jako oddělovač
