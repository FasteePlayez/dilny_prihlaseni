...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $datum_konani_raw = $_POST['datum_konani']; // Načteme jako string
    ...
    // Formát z datetime-local je 'YYYY-MM-DDTHH:MM'
    // Pro databázi potřebujeme 'YYYY-MM-DD HH:MM:SS'
    $datum_konani_dt = DateTime::createFromFormat('Y-m-d\TH:i', $datum_konani_raw);

    if (!$datum_konani_dt) {
        // Zkusíme alternativní formát, pokud prohlížeč poslal s mezerou (méně pravděpodobné pro datetime-local)
    ...
    try {
        ...
        $stmt->execute([$nazev_dilna, $vedouci, $kapacita, $cena, $datum_konani_db]); // Použijeme formátovaný datum
